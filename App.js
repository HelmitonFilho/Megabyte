...
export default function App() {
  const [nomeCategoria, setCategoria] = useState([
    {nome:'Headset',link: require('./assets/Headset.jpg'), key:1},
    {nome:'Mouse',link: require('./assets/Mouse.jpg') , key:2},
    {nome:'Teclado', link: require('./assets/Teclado.jpg'), key:3},
  ...
  const pesquisa = ()=>{
    if(pesquisaInput.toLocaleLowerCase().trim() == 'headset'){
      return <TouchableOpacity 
      onPress={() => setModalVisible(!modalVisible)}>
      <Text style={{fontSize: 20}}>Headset</Text>
      </TouchableOpacity>
    }else if(pesquisaInput.toLocaleLowerCase().trim() == 'mouse'){
      return <TouchableOpacity 
      onPress={() => setModalVisible(!modalVisible)}>
      <Text style={{fontSize: 20}}>Mouse</Text>
      </TouchableOpacity>
    }else if(pesquisaInput.toLocaleLowerCase().trim() == 'teclado' || pesquisaInput.toLocaleLowerCase().trim() == 'acessórios'){
      return <TouchableOpacity 
      onPress={() => setModalVisible(!modalVisible)}>
      <Text style={{fontSize: 20}}>Teclado</Text>
      </TouchableOpacity>
